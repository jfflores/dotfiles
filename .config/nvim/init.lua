...
vim.opt.foldlevelstart = 1                                -- start unfolded
vim.opt.foldmethod     = 'indent'                          -- not as cool as syntax, but faster
-- vim.opt.foldtext       = 'v:lua.raoulduke.foldtext()'
...
-- raoulduke.vim.setlocal(
--   'colorcolumn',
--   '+' .. raoulduke.util.join(raoulduke.util.range(0, 254), ',+')
-- )
...
if vim.fn.filereadable('/Users/raoulduke/.pyenv/shims/python') == 1 then
    -- Avoid search, speeding up start-up.
    vim.g.python3_host_prog = '/Users/raoulduke/.pyenv/shims/python'
end
...
if vim.o.loadplugins then
    raoulduke.plugin.load('LuaSnip')
    raoulduke.plugin.load('cmp-buffer')
    raoulduke.plugin.load('cmp-calc')
    raoulduke.plugin.load('cmp-cmdline')
    raoulduke.plugin.load('cmp-emoji')
    raoulduke.plugin.load('cmp-nvim-lsp')
    raoulduke.plugin.load('cmp-nvim-lua')
    raoulduke.plugin.load('cmp-path')
    raoulduke.plugin.load('cmp_luasnip')
    raoulduke.plugin.load('command-t')
    raoulduke.plugin.load('ferret')
    raoulduke.plugin.load('loupe')
    raoulduke.plugin.load('nvim-cmp')
    raoulduke.plugin.load('nvim-lspconfig')
    raoulduke.plugin.load('pinnacle')
    raoulduke.plugin.load('scalpel')
    raoulduke.plugin.load('terminus')
    raoulduke.plugin.load('vim-clipper')
    raoulduke.plugin.load('vim-easydir')
    raoulduke.plugin.load('vim-fugitive')
    -- raoulduke.plugin.load('vim-git')
    raoulduke.plugin.load('vim-repeat')
    raoulduke.plugin.load('vim-commentary')
    raoulduke.plugin.load('vim-signature')
    raoulduke.plugin.load('vim-speeddating')
    raoulduke.plugin.load('vim-surround')
    raoulduke.plugin.load('vim-tmux-navigator')
    -- raoulduke.plugin.load('vim-zsh')

    -- Lazy because it adds a slow BufEnter autocmd.
    raoulduke.plugin.lazy('nvim-tree.lua', {
        commands = {
            'NvimTreeFindFile',
            'NvimTreeToggle',
            'NvimTreeOpen',
