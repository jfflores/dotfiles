...
local command = function (name, repl, opts)
  ...
  if repl_type == 'function' then
    local key = raoulduke.util.get_key_for_fn(repl, raoulduke.g.command_callbacks)
    raoulduke.g.command_callbacks[key] = repl
    repl = 'lua raoulduke.g.command_callbacks.' .. key .. '()'
