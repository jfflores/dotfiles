...
local update_statusline = function(default, action)
  ...
  elseif filetype == 'qf' then
    if action == 'blur' then
      result =
            '%{v:lua.raoulduke.statusline.gutterpadding()}'
            .. ' '
            .. ' '
            .. ' '
            .. ' '
      ...
      result = vim.g.RaouldukeQuickfixStatusline or ''
    end
  else
    result = 1
  end
...
statusline.blur_statusline = function()
  ...
  local blurred='%{v:lua.raoulduke.statusline.gutterpadding()}'
...
statusline.set = function()
  ...
  vim.opt.statusline = ''
    ...
    .. '%{v:lua.raoulduke.statusline.lhs()}'          -- Red/green/orange modified/activity status.
    ...
    .. '%{v:lua.raoulduke.statusline.fileprefix()}'   -- Relative path to file's directory.
    ...
    .. '%{v:lua.raoulduke.statusline.filetype()}'     -- Filetype (not using %Y because I don't want caps).
    .. '%{v:lua.raoulduke.statusline.fileencoding()}' -- File-encoding if not UTF-8.
    ...
    .. '%{v:lua.raoulduke.statusline.rhs()}'          -- Line/column info.
    .. '%*'                                         -- Reset highlight group.
end
